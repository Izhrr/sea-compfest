...
const SubscriptionPage = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.mealTypes.length === 0) newErrors.mealTypes = 'Please select at least one meal type';
    if (formData.deliveryDays.length === 0) newErrors.deliveryDays = 'Please select at least one delivery day';
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSubmitStatus('error');
        setErrors({
          submit: response.message || 'Failed to submit subscription. Please try again.'
        });
      }
    } catch (error) {
      setSubmitStatus('error');
